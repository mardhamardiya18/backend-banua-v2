...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/admin/extra-menu/${id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: "Deleted!",
...
<template>
    <AppLayout title="Manage Extra Menu">
        <template #header>
                    ...
                    >
                        ...
                        <tbody>
                            ...
                            >
                                <td class="px-6 py-4">
                                    {{
                                        `${item.subproduct.portion} - ${item.subproduct.product.name}`
                                    }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ item.menuoption.name }}
                                </td>
                                <td class="px-6 py-4">
                                    +{{ formatRupiah(item.extra_price) }}
                                </td>
                                <td class="px-6 py-4">
                                    <Link
                                        :href="
                                            route('extra-menu.edit', {
                                                extra_menu: item.id,
                                            })
                                        "
